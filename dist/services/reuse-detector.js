 ...
 */
export function detectReuseCandidates(root) {
    const flat = [];
    walk(root, "root", flat);
 ...
 */
function markNode(node, targetName, candidateId) {
    const next = {
        ...node,
 ...
 */
export function annotateReuseCandidates(root, candidates) {
    let next = { ...root };
 ...
 */
export function applyReuseApprovals(root, candidates, decisions) {
    function walkApply(node) {
        const nextNode = {
            ...node,
