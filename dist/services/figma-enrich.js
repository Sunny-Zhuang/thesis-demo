...
function extractStyles(node) {
    const background = node.fills?.[0]?.color
        ? rgbaToHex(node.fills[0].color.r, node.fills[0].color.g, node.fills[0].color.b, node.fills[0].color.a)
        : undefined;
    const border = node.strokes?.[0]?.color
        ? `${node.strokeWeight ?? 1}px solid ${rgbaToHex(node.strokes[0].color.r, node.strokes[0].color.g, node.strokes[0].color.b, node.strokes[0].color.a)}`
        : undefined;
    return {
        background,
 ...
 */
function mapRawNodeToEnrichedMcp(node) {
 ...
 */
export function enrichFigmaNodes(rawData) {
