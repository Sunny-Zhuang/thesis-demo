...
import type { ScreenshotRequest, VisualAnalysis, McpStructureNode } from "../types/state.js";
import type { ReuseCandidate } from "../types/dsl.js";
import { z } from "zod";
...
interface VisionLLMResponse {
  ...
  modules?: VisualAnalysis["modules"];
  reusableCandidates?: Array<{
    name: string;
    reason: string;
    nodePaths?: string[];
  }>;
...
function normalizeReusableCandidates(raw?: VisionLLMResponse["reusableCandidates"]): ReuseCandidate[] {
...
interface CompactContextInput {
  ...
  layout?: {
    ...
    padding?: number | [number, number] | { top: number; right: number; bottom: number; left: number } | string;
    gap?: number | string;
    width?: number | string;
  ...
  children?: CompactContextInput[];
}

function buildCompactContext(root?: CompactContextInput): Record<string, unknown> | undefined {
 ...
 */
function buildCompactContextFromEnrichedNodes(nodes: McpStructureNode[]): Record<string, unknown> | undefined {
  ...
  // 转换 layout direction
  const convertDirection = (dir?: "horizontal" | "vertical"): "row" | "column" | undefined => {
    if (dir === "horizontal") return "row";
...
function inferModulesFromCompact(root?: CompactContextInput): VisualAnalysis["modules"] {
...
function mergeModulesWithCompact(
  llmModules: VisualAnalysis["modules"] | undefined,
  compactModules: VisualAnalysis["modules"]
): VisualAnalysis["modules"] {
  if (!llmModules?.length) return compactModules;
  const merged = [...llmModules];
  const usedNodeIds = new Set(
    merged.flatMap((module) => module.nodeHintIds ?? []).filter((nodeId) => !!nodeId)
  );
  const lowerNameMap = new Map(compactModules.map((item) => [item.name.toLowerCase(), item]));

  for (const module of merged) {
    if (module.nodeHintIds?.length) continue;
  ...
  for (const compactModule of compactModules) {
    ...
    if (!id || usedNodeIds.has(id)) continue;
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a senior UI analyzer. Return only valid JSON that matches this schema exactly: " +
...
async function loadEnrichedNodesForOffline(request: ScreenshotRequest): Promise<McpStructureNode[]> {
  ...
  // 默认离线路径
  pathCandidates.push(
    path.resolve("src/data/figma2code-offline/enriched-nodes.json"),
    path.resolve("src/data/figma2code-offline/08wDXrTJgBoPVvQzlkKBtV_1_124-nodes.json")
  );

  for (const filePath of pathCandidates) {
    try {
      const raw = await fs.readFile(filePath, "utf-8");
      const parsed = JSON.parse(raw) as { nodes?: McpStructureNode[] };
...
export async function analyzeScreenshot(
  ...
  // 根据模式选择数据源
  let compactContext: Record<string, unknown> | undefined;
  let root: CompactContextInput | undefined;
  ...
  if (request.offlineMode) {
    ...
    root = compact.response?.root as CompactContextInput | undefined;
  ...
  if (compactContext && compactContext.root) {
    const rootFromContext = compactContext.root as CompactContextInput;
    const childrenFromContext = (compactContext.children ?? []) as CompactContextInput[];

    // 构建一个兼容的 root 对象用于后续处理
    const rootForInference: CompactContextInput = {
      ...rootFromContext,
    ...
    return {
      ...
      reusableCandidates:
        normalizeReusableCandidates(llmAnalysis?.reusableCandidates)
    };
  }

  if (request.strictLive) {
    throw new Error(
      "strict-live enabled: failed to fetch compact live Figma structure for screenshot analysis."
    );
  }
