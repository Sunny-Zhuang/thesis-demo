...
export interface McpStructureInput {
...
type FetchSource = "figma-api" | "local-file" | "none";

interface EnrichedFetchResult {
  nodes: McpStructureNode[];
  source: FetchSource;
}

const cache = new Map<string, EnrichedFetchResult>();

/**
 * Simple fuzzy matcher between visual module name and structure node name.
 */
function moduleMatchesNode(moduleName: string, node: McpStructureNode): boolean {
 ...
 * 然后用 enrich 处理成增强节点
 */
async function fetchEnrichedNodesFromFigmaApi(
  request: ScreenshotRequest
): Promise<McpStructureNode[] | null> {
 ...
 */
async function loadNodesFromFile(path: string): Promise<McpStructureNode[]> {
  try {
 ...
 */
async function fetchEnrichedNodes(request: ScreenshotRequest): Promise<EnrichedFetchResult> {
  ...
  if (figmaNodes && figmaNodes.length > 0) {
    const result: EnrichedFetchResult = { nodes: figmaNodes, source: "figma-api" };
  ...
  const result: EnrichedFetchResult = { nodes: [], source: "none" };
  cache.set(cacheKey, result);
  return result;
}
 ...
 */
export async function fetchMcpStructure(
  input: McpStructureInput
): Promise<McpStructureNode[]> {
  ...
  if (input.request.strictLive && !apiNodes.length) {
    throw new Error(
      "strict-live enabled: no live Figma enriched nodes returned; refusing local/mock fallback."
    );
  }
  ...
  const matched: McpStructureNode[] = [];
 ...
 */
export function convertMcpStructureToMetrics(
  nodes: McpStructureNode[],
  nodeIds?: string[],
