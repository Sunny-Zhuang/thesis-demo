 ...
 */
function walk(node: DslNode, visit: (current: DslNode, parent?: DslNode) => void, parent?: DslNode): void {
  visit(node, parent);
 ...
 */
function reduceNumeric(value: number, ratio: number, min = 0): number {
 ...
 */
function applyGapRule(dsl: PageDsl, ratio: number): void {
  walk(dsl, (node) => {
 ...
 */
function applyPaddingRule(dsl: PageDsl, ratio: number): void {
  walk(dsl, (node) => {
 ...
 */
function hasButtonChild(node: DslNode): boolean {
  return (
    node.children?.some((child) => child.type === "Button" || (typeof child.ref === "string" && child.ref.includes("Button"))) ??
    false
  );
}
 ...
 */
function applyButtonAlignRule(dsl: PageDsl, align: "start" | "end"): void {
  walk(dsl, (node) => {
    if (!hasButtonChild(node)) return;
    node.layout = {
      ...(node.layout ?? {}),
      align
    };
 ...
 */
function applyFineGrainModuleSplitRule(dsl: PageDsl): void {
  walk(dsl, (node) => {
    if (!node.children?.length || node.type !== "Module") return;
    // Avoid wrapping nodes that were already split in a previous pass.
    if (node.name?.match(/Part\d+$/)) return;

    const hasNonModuleChild = node.children.some((child) => child.type !== "Module");
    if (!hasNonModuleChild) return;
...
export function applyFeedbackRules(
  inputDsl: PageDsl,
  rerunFeedbacks: string[] | undefined
): { dsl: PageDsl; appliedRules: string[] } {
  const dsl = cloneNode(inputDsl) as PageDsl;
