...
import type { PageDsl } from "./types/dsl.js";
import type { PipelineState } from "./types/state.js";
import type { McpStructureNode, ScreenshotRequest } from "./types/state.js";
 ...
 */
async function collectReviewDecisions(state: PipelineState): Promise<Record<string, boolean>> {
  const decisions: Record<string, boolean> = {};

  await withReadline(async (rl) => {
    for (const candidate of state.reuseCandidates ?? []) {
      const answer = await rl.question(
        `Approve reusable component "${candidate.name}" (${candidate.reason})? [y/N]: `
      );
 ...
 */
async function writeDslFile(dsl: PageDsl, attempt: number): Promise<string> {
...
type DslReviewResult = { approved: boolean; feedback?: string };

/**
 * Collects DSL approval result and optional rerun feedback.
 */
async function collectDslReviewDecision(dslFilePath: string): Promise<DslReviewResult> {
  return withReadline(async (rl) => {
    ...
    const feedback = await rl.question(
      "DSL rejected. Provide feedback for rerun (optional): "
    );
 ...
 */
async function runOneAttempt(initialState: PipelineState): Promise<PipelineState> {
  ...
  while (true) {
    state = (await graph.invoke(state)) as PipelineState;
    ...
    if (state.waitingForReview && state.reviewStage === "dsl") {
      const dslFilePath = await writeDslFile(state.dsl as PageDsl, (state.retryCount ?? 0) + 1);
...
type Figma2CodeRow = {
  root?: { src?: string; path?: string };
  filekey?: string;
  node_id?: string;
  page_url?: string;
  processed_metadata?: string;
};
...
async function buildRequestFromOfflineFigma2Code(
  offlinePath: string,
  strictLiveInput: boolean
): Promise<ScreenshotRequest> {
  const resolvedPath = path.resolve(offlinePath);
  const raw = await fs.readFile(resolvedPath, "utf-8");
  const parsed = JSON.parse(raw) as {
    rows?: Array<{ row?: Figma2CodeRow }>;
  };
  ...
  let mcpStructurePath: string | undefined;
...
async function run() {
  ...
  let offlinePath: string | undefined;
  ...
  if (offlineFlagIndex >= 0) {
    ...
    argsWithoutModeFlags.splice(
      offlineFlagIndex,
      maybePath && !maybePath.startsWith("--") ? 2 : 1
    );
  }
  ...
  const isUrl = (value?: string): boolean =>
    !!value && (value.startsWith("http") || value.startsWith("figma://"));
  const isJsonPath = (value?: string): boolean => !!value && value.toLowerCase().endsWith(".json");
  ...
  if (!offlinePath && !screenshotPath && !figmaUrl) {
    throw new Error(
      "Usage: npm run dev -- [--strict-live] <screenshot-path|figma-url> [mcp-structure-json-path|figma-url]\n" +
  ...
  const baseRequest: ScreenshotRequest = offlinePath
  ...
  const finalDslPath = await writeDslFile(finalState.dsl as PageDsl, (finalState.retryCount ?? 0) + 1);
